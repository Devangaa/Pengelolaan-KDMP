...
<?= $this->section('title') ?>Riwayat Transaksi Kasir<?= $this->endSection() ?>
...
<div class="space-y-6">
    <section class="rounded-3xl border border-stone-200 bg-gradient-to-r from-white via-stone-50 to-white p-6 shadow-sm">
        <div class="grid gap-6 lg:grid-cols-[1.5fr_1fr] lg:items-center">
            <div>
                <p class="text-sm font-semibold uppercase tracking-[0.24em] text-red-600">Riwayat Transaksi</p>
                <h1 class="mt-3 text-3xl font-semibold tracking-tight text-stone-900 sm:text-4xl">Riwayat Transaksi </h1>
                <p class="mt-4 max-w-2xl text-stone-600">Menampilkan transaksi yang Anda buat sendiri dengan filter tanggal dan urutan.</p>
            </div>
        </div>
    </section>

    <section class="rounded-2xl border border-red-100 bg-white p-4 shadow-sm sm:p-6">
        <form id="transactionFilterForm" action="<?= base_url('transaksi') ?>" method="GET">
            <div class="space-y-4">
                <div class="grid gap-4 lg:grid-cols-3">
                    ...
                    <div class="min-w-[180px] w-full">
                        ...
                        <div class="relative" data-datepicker data-datepicker-name="startDate">
                            ...
                            <button type="button" data-datepicker-toggle class="w-full rounded-xl border border-stone-200 bg-stone-50 px-3 py-2.5 pr-10 text-sm font-medium text-stone-700 shadow-sm text-left transition focus:border-red-400 focus:outline-none focus:ring-2 focus:ring-red-100">
                                <span data-datepicker-value class="<?= empty($startDate) ? 'text-stone-400' : '' ?>">Pilih tanggal</span>
                                <span class="pointer-events-none absolute inset-y-0 right-3 flex items-center text-stone-500">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    <div class="min-w-[180px] w-full">
                        ...
                        <div class="relative" data-datepicker data-datepicker-name="endDate">
                            ...
                            <button type="button" data-datepicker-toggle class="w-full rounded-xl border border-stone-200 bg-stone-50 px-3 py-2.5 pr-10 text-sm font-medium text-stone-700 shadow-sm text-left transition focus:border-red-400 focus:outline-none focus:ring-2 focus:ring-red-100">
                                <span data-datepicker-value class="<?= empty($endDate) ? 'text-stone-400' : '' ?>">Pilih tanggal</span>
                                <span class="pointer-events-none absolute inset-y-0 right-3 flex items-center text-stone-500">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                    </svg>
    ...
    <section class="rounded-3xl border border-stone-200 bg-white p-6 shadow-sm" id="transactionTableContainer">
        <?= view('cashier/partials/transaction_table', ['transactions' => $transactions, 'pager' => $pager ?? null]) ?>
    </section>
</div>
